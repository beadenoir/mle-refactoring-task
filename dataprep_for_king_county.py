...
def fill_nan_with_zero(df:pd.DataFrame, column_name:str) -> pd.DataFrame:
    if column_name in ['view', 'waterfront']:
...
def drop_needless_columns(df:pd.DataFrame, column_name:str) -> pd.DataFrame:
    if column_name in ['yr_renovated', 'yr_built']:
        df.drop(column_name, axis=1, inplace=True, errors='ignore')
    return df

def create_sqft_price(df:pd.DataFrame) -> pd.DataFrame:
